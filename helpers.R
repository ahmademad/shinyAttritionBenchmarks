...
data <- read.csv("./data/abs_benchmarktool_sourcedata_public.csv", stringsAsFactors = FALSE, 
                 sep = "\t")
description[, c("Notation", "Level", "Meaning")] <- sapply(description[, 
                                                                       c("Notation", "Level", "Meaning")], trimws)
...
data[, c("period", "subgrp", "interType", "group1", "group2", "group3")] <- sapply(data[, 
                                                                                        c("period", "subgrp", "interType", "group1", "group2", "group3")], 
                                                                                   trimws)
transitions <- unique(data$period)

meaning <- function(g1, type) {
  temp1 <- description[description$Type == type & description$Level == 
                         g1, c("Notation", "Meaning")]
  temp1 <- unique(temp1)
